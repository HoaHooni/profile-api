...
public class ProjectDto extends BaseDto<Long> {
	...
	public static ProjectDto convertToDto(ProjectEntity entity) {
		...
		return ProjectDto.builder().title(entity.getTitle()).description(entity.getDescription())
				.responisble(entity.getResponisble()).langages(entity.getLanguages()).other(entity.getOther())
	...
	public static List<ProjectDto> convertToDtos(List<ProjectEntity> entitys) {
		if (entitys == null || entitys.size() == 0)
			return null;
		List<ProjectDto> projectDtos = entitys.stream().map(item -> ProjectDto.convertToDto(item)).collect(Collectors.toList());
		return projectDtos;
	}
	...
	public static ProjectEntity convertToEntity(ProjectDto projectDto, Long userId) {
		...
		return ProjectEntity.builder().title(projectDto.getTitle()).description(projectDto.getDescription())
				.responisble(projectDto.getResponisble()).languages(projectDto.getLangages())
